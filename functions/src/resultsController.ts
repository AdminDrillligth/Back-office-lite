...
let saveIndataBase = async (uuid:any, result:any,coach:any)=>{
  functions.logger.log("DATA DECODED RESULT TO SAVE : ! ", result)
  const results_handler = db.collection('results-handler');
 
  results_handler.doc(uuid).set({result:result, idExercice:result.infos.idExercise, idResult:uuid, idAccount:coach.id }).then( async (ref:any) => {
